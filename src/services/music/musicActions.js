 ...
 * /play query: Drake God's Plan
 * /play query: https://youtube.com/...
 *
 * If the user gives a normal song name, the bot automatically
 * searches YouTube Music using ytmsearch.
 */
export async function playQuery(client, interaction, query) {
    ...
    try {
        ...
        throw new TitanBotError(
            'Music search failed',
            ErrorTypes.EXTERNAL,
            '❌ I could not search for that music. Please try another song.',
        );
    }
    ...
    console.log(
        `[MUSIC] Load type: ${loadType}, tracks: ${tracks.length}`,
    );
